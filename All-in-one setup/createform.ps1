...
function Get-ErrorMessage {
    ...
    process {
        ...
        $auditMessage = $ErrorObject.Exception.Message
        
        # Try to get more details from ErrorDetails if available
        if (-not [string]::IsNullOrEmpty($ErrorObject.ErrorDetails.Message)) {
            $errorMessage = $ErrorObject.ErrorDetails.Message
            $auditMessage = $ErrorObject.ErrorDetails.Message
        }
        
        # For web exceptions, add status information
        if ($ErrorObject.Exception.GetType().FullName -eq 'System.Net.WebException') {
            if ($null -ne $ErrorObject.Exception.Response) {
                $errorMessage += " [Status: $($ErrorObject.Exception.Response.StatusCode)]"
                $auditMessage += " [Status: $($ErrorObject.Exception.Response.StatusCode)]"
            }
        }
        ...
        if (-not [string]::IsNullOrEmpty($errorMessage)) {
            try {
                $jsonError = $errorMessage | ConvertFrom-Json -ErrorAction SilentlyContinue
                if ($null -ne $jsonError -and $null -ne $jsonError.error) {
                    $errorMessage = $jsonError.error
                    if ($null -ne $jsonError.error.message) {
                        $auditMessage = $jsonError.error.message
                    } else {
                        $auditMessage = $jsonError.error
                    }
                }
            }
            catch {
        ...
        [PSCustomObject]@{
            ...
            VerboseErrorMessage = $errorMessage
            AuditErrorMessage   = $auditMessage
        }
    }
}
